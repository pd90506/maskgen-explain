...
class MaskGeneratingModel(nn.Module):
    ...
    def loss_func(self, sim, mask_list, reverse_mask_list, masked_prob_list, reverse_masked_prob_list):
        ...
        scaler = mask_sample_probs / (reverse_mask_sample_probs + mask_sample_probs + 1e-5) # [N, n_steps, 1]


        selected_samples = mask_samples # * selector + reverse_mask_samples * (1 - selector) # [N, n_steps, L]
        selected_mask_sample_probs = mask_sample_probs * scaler * self.logit_scale.exp()  # + reverse_mask_sample_probs * (1 - selector) # [N, n_steps, 1]
        selected_mask_sample_probs = torch.softmax(selected_mask_sample_probs, dim=1) # [N, n_steps, 1]

        reward_loss = bce_loss(mask_prob, selected_samples) # [N, n_steps, L]
        reward_loss = (reward_loss * selected_mask_sample_probs).sum(1).mean() # [N, n_steps, L] -> [N, 1, L] -> scalar

        # mask_loss
        mask_loss = (mask_prob * mask_prob).mean()
    ...
    def generate_mask(self, sim):
        ...
        with torch.no_grad():
            ...
            mask_prob = torch.clamp(mask_prob, 0.2, 0.8) # prevent the mask_prob from being too close to 0 or 1
    ...
    def train_one_batch(self, x: torch.Tensor, optimizer: torch.optim.Optimizer, n_steps=10):
        ...
        for idx in range(n_steps):
            ...
            mask, masked_probs, reverse_mask, reverse_masked_probs = self.sample_one_step(x, sim, predicted_class_selector)
