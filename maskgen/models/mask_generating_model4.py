...
class MaskGeneratingModel(nn.Module):
    ...
    def loss_func(self, sim, mask_list, masked_prob_list, mode='ins'):
        ...
        if mode == 'ins':
            ...
            reward_loss = bce_loss(mask_prob, 1.0 - mask_samples) # [N, n_steps, L]
            reward_loss = (reward_loss * (1. - mask_sample_probs)).mean()# [N, n_steps, L]
            print('in del mode.')
        # mask_loss
    ...
    def sample_one_step(self, x: torch.Tensor, sim: torch.Tensor, predicted_class_selector: torch.Tensor, mode='ins'):
        with torch.no_grad():
    ...
    def train_one_step(self, x: torch.Tensor, optimizer: torch.optim.Optimizer, n_steps=10, mode='ins'):
        self.train()
        optimizer.zero_grad()
        ...
        for idx in range(n_steps):
            mask, masked_probs = self.sample_one_step(x, sim, predicted_class_selector, mode=mode)
            mask_list.append(mask)
            masked_probs_list.append(masked_probs)
        
        loss_dict = self.loss_func(sim, mask_list, masked_probs_list, mode=mode)
    ...
    def train_insertion_step(self, x: torch.Tensor, optimizer: torch.optim.Optimizer, n_steps=10):
        return self.train_one_step(x, optimizer, n_steps=n_steps, mode='ins')
    
    def train_deletion_step(self, x: torch.Tensor, optimizer: torch.optim.Optimizer, n_steps=10):
        return self.train_one_step(x, optimizer, n_steps=n_steps, mode='del')
    
    def train_one_batch(self, x: torch.Tensor, optimizer: torch.optim.Optimizer, n_steps=10):
        del_loss_dict = self.train_deletion_step(x, optimizer, n_steps=n_steps)
        ins_loss_dict = self.train_insertion_step(x, optimizer, n_steps=n_steps)

        return {'loss': del_loss_dict['loss'], 
                'reward_loss': ins_loss_dict['reward_loss'],
                'regret_loss': del_loss_dict['reward_loss'],
                'mask_loss': del_loss_dict['mask_loss']}
