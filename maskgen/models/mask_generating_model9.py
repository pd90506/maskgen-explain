# Yeild consistent convergence. moded: loss: 
        # mask_loss = mask_prob * mask_sample_probs * bce_loss(1 - mask_prob, mask_samples) # [N, n_steps, L]
...
class SimilarityMeasure(nn.Module):
    def __init__(self, pred_hidden_size, explain_hidden_size, embed_size=512):
        ...
        self.attention = nn.MultiheadAttention(embed_size, num_heads=8, dropout=0.1)
    ...
    def forward(self, pred_feature, explain_features):
        ...
        """
        pred_feature = F.normalize(self.pred_mapp(pred_feature), p=2, dim=-1)  # [N, embed_size]
        explain_features = F.normalize(self.explain_map(explain_features), p=2, dim=-1)  # [N, L, embed_size]

        # Concat pred_feature and explain_features, the first token is the pred_feature
        hidden_states = torch.cat([pred_feature.unsqueeze(1), explain_features], dim=1)  # [N, L+1, embed_size]
        hidden_states = self.attention(hidden_states, hidden_states, hidden_states)[0]  # [N, L+1, embed_size]

        pred_feature = F.normalize(hidden_states[:, 0:1, :],p=2, dim=-1)  # [N, 1, embed_size]
        explain_features = F.normalize(hidden_states[:, 1:, :], p=2, dim=-1)  # [N, L, embed_size]
...
class MaskGeneratingModel(nn.Module):
    ...
    def loss_func(self, sim, mask_list, masked_prob_list):
        ...
        # if reverse_mask_sample_probs is higher, we want to optimize the probability of generating the reverse masks

        # scaled_mask_sample_probs = mask_sample_probs #* self.logit_scale.exp()  # + reverse_mask_sample_probs * (1 - selector) # [N, n_steps, 1]
        # scaled_mask_sample_probs = torch.softmax(scaled_mask_sample_probs, dim=1) # [N, n_steps, 1]

        reward = bce_loss(mask_prob, mask_samples) # [N, n_steps, L]
        # reward_loss = (reward_loss * mask_sample_probs).mean(1).mean()
        reward_loss = (reward * mask_sample_probs).mean(-1) #/ (1e-5 + mask_sample_probs.mean(-1)) # [N, n_steps]
        reward_loss = reward_loss.mean()

        # mask_loss
        mask_mean = mask_prob.mean([1, 2]) # [N]
        prob_mean = mask_sample_probs.mean([1, 2]) # [N]
        # mask_loss = ((1.2 - mask_mean - prob_mean) ** 2).mean() # [N, L] -> [N]
        mask_loss = mask_prob * mask_sample_probs * bce_loss(1 - mask_prob, mask_samples) # [N, n_steps, L]
        mask_loss = mask_loss.mean()
        # mask_loss = torch.relu(mask_mean - (1 - prob_mean)).mean()
        # mask_loss = mask_prob.mean()
        # alt_mask_loss = ((0.2 - mask_mean - prob_mean) ** 2).mean() # [N, L] -> [N]
