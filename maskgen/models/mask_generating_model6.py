...
class MaskGeneratingModel(nn.Module):
    ...
    def loss_func(self, sim, mask_list, reverse_mask_list, masked_prob_list, reverse_masked_prob_list):
        ...
        selector = (mask_sample_probs > reverse_mask_sample_probs).float() # [N, n_steps, 1]

        selected_samples = mask_samples # * selector + reverse_mask_samples * (1 - selector) # [N, n_steps, L]
        selected_mask_sample_probs = mask_sample_probs * selector  # + reverse_mask_sample_probs * (1 - selector) # [N, n_steps, 1]

        reward_loss = bce_loss(mask_prob, selected_samples) # [N, n_steps, L]
        reward_loss = (reward_loss * selected_mask_sample_probs).sum() / (L * (selected_mask_sample_probs + 0.1).sum())  # [N, n_steps, L]
